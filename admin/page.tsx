...
export default function AdminPage() {
  ...
  const handleDeleteAll = async () => {
    if (!confirm(`Are you sure you want to delete ALL ${selectedCategory}? This cannot be undone!`)) return
    if (!confirm('REALLY sure? This will delete everything!')) return
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            <div className={`mt-4 p-4 rounded-lg ${importResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
              {importResult.success ? (
                <div className="flex items-center gap-2 text-green-700">
                  ...
                  Imported {importResult.added} products ({importResult.errors} errors)
                </div>
              ) : (
        ...
        <div className="bg-white rounded-lg border overflow-hidden">
          <div className="p-4 border-b flex justify-between items-center">
            <h2 className="font-semibold">{selectedCategory} ({appliances.length} shown)</h2>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="p-8 text-center text-slate-500">
              No products in this category. Add some or import a CSV!
            </div>
          ) : (
        ...
        <div className="mt-6 bg-white rounded-lg border p-6">
          <h2 className="font-semibold mb-2">CSV Import Format</h2>
          <p className="text-sm text-slate-600 mb-4">
            Your CSV should have headers matching the database columns. At minimum include:
          </p>
          <code className="block bg-slate-100 p-3 rounded text-sm">
            category,brand,model,title,price,type,product_url,image_url
          </code>
          <p className="text-sm text-slate-500 mt-2">
            Valid categories: Refrigerators, Washers, Dryers, Air Fryers, Ice Makers, Window AC, Freezers
          </p>
        </div>
      </div>
    </div>
